...
<aside class="col-lg-4 pt-4 pt-lg-2 px-max-md-0 order-summery-aside">
    ...
    <input class="form-check-input" type="checkbox" id="termsCheck" name="termsCheck" required checked>
    <label class="form-check-label" for="termsCheck">
        {{ translate('I agree to the') }}
        <a href="{{route('terms')}}" class="text-danger" target="_blank">{{ translate('terms and conditions') }}</a>,
        <a href="{{route('return-policy')}}" class="text-danger" target="_blank">{{ translate('return policy') }}</a>
        {{ translate('&') }}
        <a href="{{route('privacy-policy')}}" class="text-danger" target="_blank">{{ translate('privacy policy') }}</a>
    </label>
</div>

@endif
            <a id="checkoutButton" class="btn btn--primary btn-block  {{$cart->count() <= 0 ? 'custom-disabled' : ''}} action-checkout-function">অর্ডার করতে এগিয়ে যান</a>
        </div>
...
<div class="bottom-sticky3 bg-white p-3 shadow-sm w-100 d-lg-none">
    <div class="d-flex justify-content-center align-items-center fs-14 mb-2">
     ...
     @if(isset($step) && $step == 3)
    <div class="form-check mt-3">
    <input class="form-check-input termsCheck" type="checkbox" id="termsCheckm" name="termsCheckm" required checked>
    <label class="form-check-label" for="termsCheckm">
        {{ translate('I agree to the') }}
        <a href="{{route('terms')}}" class="text-danger" target="_blank">{{ translate('terms and conditions') }}</a>,
        <a href="{{route('return-policy')}}" class="text-danger" target="_blank">{{ translate('return policy') }}</a>
        {{ translate('&') }}
...
@endif
    <a id="checkoutButtonm" data-route="{{ Route::currentRouteName() }}"
       class="checkoutButton btn btn--primary btn-block  text-capitalize  {{$cart->count() <= 0 ? 'custom-disabled' : ''}} action-checkout-function">অর্ডার করতে এগিয়ে যান</a>
</div>
