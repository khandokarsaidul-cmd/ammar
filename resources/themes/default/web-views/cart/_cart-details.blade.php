...
@php($miptotal=\App\Utils\CartManager::cart_total_applied_discount($cartList))
...
@push('script')
	...
	dataLayer.push({
    	...
    	ecommerce: {
            ...
            value: "{{$miptotal}}",
            @isset($group)
        	items: [@foreach ($group as $product){
                item_name 	: "{{$product->name}}",
                item_id   	: "{{$product->id}}",
                price     	: parseFloat("{{$product->price-$product->discount}}".replace(/[^0-9.-]/g, '')),
