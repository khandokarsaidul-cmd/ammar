...
function shipping_method_select(get_value){
    ...
    $('#district').val(shipping_value.district).trigger('change');
    $('#address_type').val(shipping_value.address_type);
    $(document).one('thana_loaded', function(){
        $('#thana').val(shipping_value.area).trigger('change');
        $('#thana.selectpicker').selectpicker('refresh');
    });
