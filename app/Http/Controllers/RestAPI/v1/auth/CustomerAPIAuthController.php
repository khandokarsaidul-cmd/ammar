...
class CustomerAPIAuthController extends Controller
{
    ...
    public function register(Request $request): JsonResponse
    {
        ...
        $this->manageAppRegisterPoint($user);
    ...
    private function manageAppRegisterPoint($user){
        $settings = BusinessSetting::whereIn('type', [
            'app_register_point_start_date',
            'app_register_point_end_date',
            'app_register_earning',
            'app_register_point',
        ])->pluck('value', 'type');

        $earningEnabled = (int) ($settings['app_register_earning'] ?? 0);
        $startDate = $settings['app_register_point_start_date'] ?? null;
        $endDate = $settings['app_register_point_end_date'] ?? null;
        $point = (float) ($settings['app_register_point'] ?? 0);

        if (!$earningEnabled || !$startDate || !$endDate) {
            return;
        }

        $now = Carbon::now();
        if ($now->between(Carbon::parse($startDate), Carbon::parse($endDate))) {
            $user->wallet_balance += $point;
            $user->save();
        }
    }
