   ...
   public function getCashOnDeliveryCheckoutComplete(Request $request): View|RedirectResponse|JsonResponse
{
    try {
        ...
        // 3️⃣ Verify minimum order amount
        ...
        // 4️⃣ Handle new customer registration
        if (session('newCustomerRegister')) {
            ...
            $newUser = User::create([
                ...
                'referral_code' => $newCustomerRegister['referral_code'] ?? null,
            ]);

            session()->put('newRegisterCustomerInfo', $newUser);

            $guestID = session('guest_id', 0);
            ShippingAddress::where(['customer_id' => $guestID, 'is_guest' => 1])
                ->whereIn('id', [session('address_id'), session('billing_address_id')])
                ->update(['customer_id' => $newUser->id, 'is_guest' => 0]);
        }
        ...
        // 6️⃣ Clean cart and sessions
        CartManager::cart_clean();
        $isNewCustomer = session('newCustomerRegister');
        ...
        return view(VIEW_FILE_NAMES['order_complete'], [
            'order_ids' => $orderIds,
            'isNewCustomerInSession' => $isNewCustomer,
        ]);
...
public function sendOrderSms($orderId)
{
    ...
    $rawPhone = $shippingData->phone ?? null;

    if (!$rawPhone) {
        return response()->json([
            'status' => 'error',
            'message' => 'Phone number not found in shipping data'
        ]);
    }


     $message = "Dear {$shippingData->contact_person_name},Your Order Has been Placed, Order No- {$order->id} and Total Bill {$order->order_amount} TK.Contact 01975553300. 
Thanks by";

  
    $apiKey = '0v2YWgygpxw2olBkQq822syS8B67Okm6z20TKCOR';
    $apiUrl = "https://api.sms.net.bd/sendsms?api_key={$apiKey}&msg=" . urlencode($message) . "&to=" . urlencode($rawPhone);
    ...
    public function generateFacebookFeed()
{
    $disprice=null;
    $products = Product::where('status', 1)->get(); 
    $csvHeader = ['id', 'title', 'description', 'availability', 'condition', 'price', 'sale_price', 'link', 'image_link', 'brand', 'google_product_category', 'fb_product_category'];

    $csvData = [];
    foreach ($products as $product) {
        
        $cat=DB::table('categories')->where('id', $product->category_id)->value('name');
        $brandn=DB::table('brands')->where('id', $product->brand_id)->value('name');
        
        if($product->discount>0){
            if($product->discount_type=="percent"){
            $disprice=$product->unit_price-($product->unit_price*$product->discount/100);
            }else{
            $disprice=$product->unit_price-$product->discount;
        }
        }
    
        
        $imageUrl = $product->thumbnail ? url('storage/app/public/product/thumbnail/' . $product->thumbnail) : '';

        $csvData[] = [
            $product->id,
            $product->name,
            strip_tags($product->details) ?: 'No description available', 
            'in stock',  
            'new',       
            $product->unit_price,       
            $disprice ?: $product->unit_price,  
