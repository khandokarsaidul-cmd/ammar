    ...
    public function getDefaultTheme(string $slug): View|RedirectResponse
    {
        ...
        if ($product) {
            ...
            $customData = [
                ...
                'value' => $product->unit_price, // Price of the viewed product
    'currency' => 'BDT', // Currency code (USD, EUR, etc.)
    'content_category' => $product->category?->name, // Category of the product (optional)
    'num_items' => 1, // Number of items being viewed (can be more if viewing multiple products)
            ];
        
            $response = $facebookService->sendEvent('ViewContent', $userData, $customData);
            if ($response['events_received'] ?? 0 > 0) {
                \Log::info('Facebook Conversion API: ViewContent Event successfully sent.', $response);
            } else {
                \Log::error('Facebook Conversion API: ViewContent Failed to send event.', $response);
            }
