...
class BrandController extends BaseController
{
     ...
     */
     public function colorindex(Request $request){
        ...
        $code = null;
          ...
          $colors=DB::table('colors')->orderBy('id','DESC');
          if ($request->name != null) {
            $colors = $colors->where('name', $request->name);
            $name = $request->name;
        }
        
        if ($request->code != null) {
            $colors = $colors->where('code', $request->code);
            $code = $request->code;
        }
          
          $colors=$colors->paginate(10);
          
          return view('admin-views.brand.colorlist',compact('colors'));
        }
        ...
        return view('admin-views.brand.coloredit',compact('color'));
        }
    ...
    public function updatecolor(Request $request)
    {
        ...
        DB::table('colors')->where('id',$request->cid)->update($data);
        Toastr::success("Color Updated Successfully");
        return redirect()->route('admin.color.colorlist');
    }
