...
class OrderController extends BaseController
{
    ...
    public function draftOrder(){
        $cartIds = DB::table('carts')->whereNotNull('phone')
        ->select(DB::raw('MAX(id) as id'))
        ->groupBy('customer_id');
         
          $orders= DB::table('carts')->whereIn('id', $cartIds)->paginate(10);
          //return $orders->count();
           return view('admin-views.order.draftlist',compact('orders'));
    }
    
     public function draftdestroy($customer_id)
        {
           $color=DB::table('carts')->where('customer_id',$customer_id)->delete();
         Toastr::success("Draft Order Deleted Successfully");
    ...
    ): JsonResponse
    {
        ...
        if ($request['order_status'] == 'delivered' && $order['seller_id'] != null) {
         ...
         $this->sendOrderSms($request['id'],$request['order_status']);
...
public function sendOrderSms($orderId,$ostatus)
{
...
if($ostatus!="delivered"){
    $message = "Dear {$shippingData->contact_person_name},Your Order No- {$order->id} Has been {$ostatus},  and Total Bill {$order->order_amount} TK.Contact 01975553300. 
Thanks by";
} else{
    $message = "Dear {$shippingData->contact_person_name},Your Order No- {$order->id} Has been {$ostatus},  and Total Bill Received {$order->order_amount} TK. 
Thanks by";
}
