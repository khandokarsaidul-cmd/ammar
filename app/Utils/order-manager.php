...
class OrderManager
{
    ...
    public static function generate_order($data)
    {
        ...
        if(isset($userCheck)){
            $userData = [
                    'em' => hash('sha256', $userCheck->email), // Email hashed with SHA256
                    'ph' => hash('sha256', $userCheck->phone), // Phone hashed with SHA256
                    'fn' => isset($userCheck['f_name']) ? hash('sha256', $userCheck['f_name']) : null,
                        'ln' => isset($userCheck['l_name']) ? hash('sha256', $userCheck['l_name']) : null,
            ...
            $userData = [
                    'em' => hash('sha256', 'guestemail@gmail.com'), // Email hashed with SHA256
                    'ph' => hash('sha256', 'guestphone'), // Phone hashed with SHA256
            ...
            $customData = [
                'content_ids' => $order->details->pluck('product_id')->toArray(), // Product ID(s) for the purchase
                'content_name' => $product_name, // Product Name(s)
                'content_type' => 'product', // Type of content (can also be 'service', etc.)
                'value' => $order->order_amount,
                'currency' => 'BDT', // Currency used for the transaction (e.g., 'USD', 'BDT')
                'num_items' => $order->details->count(), // Total number of items in the transaction (if applicable)
                'payment_method' => $order->payment_method, // Payment method (e.g., 'credit_card', 'paypal', etc.)
                'order_id' => $order->id, // Unique order ID for the transaction
                'shipping' => $order->shipping_cost, // Shipping cost, if applicable
                'tax' => 0 // Tax amount, if applicable
            ];

            
            $response = $facebookService->sendEvent('Purchase', $userData, $customData);
            
            if ($response['events_received'] ?? 0 > 0) {
                \Log::info('Facebook Conversion API: Purchase Event successfully sent.', $response);
            } else {
                \Log::error('Facebook Conversion API: Purchase Failed to send event.', $response);
            }
