        ...
        $facebookService = new FacebookConversionService();
            if($user != 'offline'){
                    $userData = [
                        ...
                        'fn' => isset($user['f_name']) ? hash('sha256', $user['f_name']) : null,
                        'ln' => isset($user['l_name']) ? hash('sha256', $user['l_name']) : null,
            ...
            $response = $facebookService->sendEvent('AddToCart', $userData, $customData);
            
             if ($response['events_received'] ?? 0 > 0) {
                \Log::info('Facebook Conversion API: AddToCart Event successfully sent.', $response);
            } else {
                \Log::error('Facebook Conversion API: AddToCart Failed to send event.', $response);
            }
